...
d_max <- function(x, y, w, span, j) {
 ...
 n <- length(y)
  ...
  if (j<=50) {
    #not too far away
    index_max_all_maj <- subset(index_max_all, index_max_all<=(peaks$i+18))
    #not too far back
  ...
  if (j>50) {
    #not too far away
    index_max_all_maj <- subset(index_max_all, index_max_all<=(peaks$i+11))
    #not too far back
    index_max_all_maj <- subset(index_max_all_maj, index_max_all_maj>=(peaks$i-2))
  }
  ...
  #at late times, I don't want the peak to move from the spot unless the peak is above the treshold
  
  if (j>160) {
  
    if (max_value$intensity<(baseline[1]+base_sd[1])) { #if peak detected in below surface
      
      index_max_all_maj <- peaks$i #remain where you were before
  ...
  df_T
              ...
              ends_sink <- cumsum(sinking$lengths)
              starts_sink <- ends - sinking$lengths+1
              ends_sink
              
              starts_sink
              
              df_sink <- data.frame(xmin=starts_sink, xmax=ends_sink, type=sinking$values)
              df_sink
              
              df_T_sink<- df_sink[df_sink$type == TRUE, ] 
              df_T_sink
    ...
    
  {
    ...
    long_int <- which(df_w$wide>4) #which are the long intervals
  ...
  if(j>15) {
    if (front_index>(peaks$front_ind+15)) {
      front_index <- peaks$front_ind
    }
  }
  ...
  #if there is nothing sinking below the baseline (as it may happen at late time) stay in the lowest part of the immediate neighbourhood
              
              if (dim(df_T_sink)[1] == 0) { #if there is nothing below the baseline
                
                #consider the surroundings
                
                min_neighbour <- peaks$front_ind - 5
                
                max_neighbour <- peaks$front_ind + 5
                
                
                #subsets the df to extract data fo the surroundings neighbour intervall
                dip_values_all <- subset(df_y_smooth_trimmed, index_num < max_neighbour)
                
                dip_values_all <- subset(dip_values_all, index_num > min_neighbour)
                
                #find smallest min in the neighbourhood
                min_value_dip <- min(dip_values_all$intensity)
                
                #subset dataframe to have a row that contains both the value of the chosen min and the index of it
                min_value <- df_y_smooth_trimmed[which(intensity==min_value_dip),]
                
                front_index <- min_value$index_num
      ...
      #the front should never go back
...
if (j<100) { #the constraint of the front on the peak position is now only for early time points, as the baseline here changes over time and front position is not reliable anymore at late time points
...
wave_profile <- function(PLANT, GEN) {
  ...
  #cut away anything that is before the veins are visible
  dftrim <- subset(df, frame>=visibleveins)
  dftrim <- dftrim[-1]
  head(dftrim, 10)
  ...
  dftrimt0 <- subset(dftrimt0, t<=400) 
...
folders <- c(genotype1, genotype2)#, genotype3, genotype4)
...
for (f in folders) {
 ...
 # NAME <- paste(GEN," ", "series"," ", PLANT, sep="")
  ...
  for (k in plants) { #with this small loop I feed my function
    ...
    visibleveins <- feed_fun[which(feed_fun$pl==k), "vv"]
    ...
    for (j in times) {
      ...
      fit_loess_peaks (w, span, j, SE, x, profile_t)
